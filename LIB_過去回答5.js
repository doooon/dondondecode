...
// @a! c! !c d! #b !a b a! !a ! b !a!a c@ d! d! a# a
if (
  (
  TEXT.match(/^[a-e1-5]{1,4}([\s0f69\-.,:;|\\&+])+([a-e1-5]{1,4}\1+)+[a-e1-5]{1,4}\1*$/i) || 
  TEXT.match(/^[a-e!@#$%]{1,4}([\s0f69\-.,:;|\\&+])+([a-e!@#$%]{1,4}\1+)+[a-e!@#$%]{1,4}\1*$/i)
  ) && 
  kouseimoji.length>=3
) {
  htmlTmp.push(TEXT);
...
if (
  ...
  htmlTmp.push(TEXT);
  htmlTmp.push("<b>(!@#$%を.....、12345を-----でモールス)</b>");
  var tmpRE=
    new RegExp(RegExp.$1, "g");
  var tmp=TEXT.replace(tmpRE, " ");
  tmp=tmp.replace(/ +/g," ")
...
function phonetic(str) {
  ...
  phoneticL=[["Alfa","A"],["Alpha?","A"],["Bravo?","B"],["Char(lie)?","C"],["Delta?","D"],["Echo","E"],["Foxt(rot)?","F"],["Golf","G"],["Hotel?","H"],["India?","I"],["Juli(ett?)?","J"],["Kilo","K"],["Lima","L"],["Mike","M"],["Nove(mber)?","N"],["Oscar?","O"],["Papa","P"],["Queb(ec)?","Q"],["Romeo?","R"],["Sier(ra)?","S"],["Tango?","T"],["Unif(orm)?","U"],["Vict(or)?","V"],["Whis(key)?","W"],["(X-ray|Xray)","X"],["Yank(ee)?","Y"],["Zulu","Z"]];
