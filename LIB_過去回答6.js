...
if (
  ...
  for (var i=0; i<tmp.length; i++) {
    ...
    htmlTmp.push(`[${i}] ${tmp[i]}+${j}= ${Number(tmp[i])+j} (${j}= ${tmp.length} - ${i})`);
  }
  htmlCode(result.join(""));
  ...
  for (var i=0; i<tmp.length; i++) {
    ...
    result.push(Number(tmp[i])+j);
    htmlTmp.push(`[${i}] ${tmp[i]}+${j}= ${Number(tmp[i])+j} (${j}= ${tmp.length-1} - ${i})`);
  }
  htmlCode(result.join(""));
  htmlTmp.push("==============");
}
...
  }
  ...
  htmlTmp.push("(012abc)");
  htmlTmp.push(to012abc(result2.join(" ")));
  htmlCode(to012abc(result2.join(" ")));
  htmlTmp.push("(123abc)");
  htmlTmp.push(to123abc(result2.join(" ")));
