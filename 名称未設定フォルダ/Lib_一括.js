...
if (TEXT.match(/^[a-z]+$/i)) {
  ...
  htmlCode(tmp);
  if (tmp.length > 1) htmlTmp.push(tmp.match(/../g).join(" "));
  ...
  if (tmp.length > 1) htmlTmp.push(strReverse(tmp).match(/../g).join(" "));
...
//vigenereデコード
if (TEXT.match(/^(\d*[a-z]*\d*)+$/i)) {
  htmlTmp.push("(vigenere)");
  
  function vige(key) {
    htmlTmp.push("(vig key: "+key+")");
    var tmpv=vigenereDec(TEXT, key);
    htmlCode(tmpv);
  }
...
function goMorse(sorce) {
    ...
    function douon(str, rep, sorce) {
       // alert(str);
       // alert(rep);
       // alert(sorce);
        var strRE=new RegExp(str, "ig");
        if (sorce.match(strRE)) {
            htmlCode(sorce.replace(strRE, rep));
        }
    }
    ...
    htmlTmp.push("(more atbash19 & reverse)");
    htmlCode(atbash19(strReverse(demrs)));
    douon("sea", "c", atbash19(strReverse(demrs)));
    douon("see", "c", atbash19(strReverse(demrs)));
    
    htmlTmp.push("-----------");

    htmlTmp.push("(reverse)");
    var rev=morse.split("").reverse().join("");
    //htmlTmp.push(rev);
    htmlTmp.push("(de-morse)");
    var demrs=morseExchange(rev);
    htmlCode(demrs);
    // c, j, zはreverse文字がないので同音で隠されてるかも
    htmlTmp.push("reverse出来ない3文字→ C J Z");
    douon("sea", "c", demrs);
    douon("see", "c", demrs);
    ...
    htmlTmp.push("swap出来ない4文字→ C H V Z");
    douon("sea", "c", demrs);
    douon("see", "c", demrs);
    ...
    htmlTmp.push("(more reverse & atbash19)");
    htmlCode(strReverse(atbash19(demrs)));
    douon("sea", "c", strReverse(atbash19(demrs)));
    douon("see", "c", strReverse(atbash19(demrs)));

    htmlTmp.push("-----------");

    htmlTmp.push("(swap & reverse)");
    var swpRev;
    swpRev=swap.split("").reverse().join("");
    //htmlTmp.push(swpRev);
    htmlTmp.push("(de-morse)");
    var demrs=morseExchange(swpRev);
    htmlCode(demrs);
    // b, h, は反転文字がないので同音で隠されてるかも
    htmlTmp.push("swap&reverse出来ない2文字→ B H");
    htmlTmp.push("(more reverse)");
    htmlCode(strReverse(demrs));
    htmlTmp.push("(more atbash19)");
    htmlCode(atbash19(demrs));
    htmlTmp.push("(more atbash 1<->9 & reverse)");
    htmlCode(strReverse(atbash19(demrs)));
  
    htmlTmp.push("-----------");
  
    // モールスから短符長符数でdecASCII
    if (morse.match(/\./) && morse.match(/-/)) {
        var morsejoin=morse.replace(/[\s]/g, "");
        var tmpL=[];
        if (morsejoin.match(/^\./)) {
    ...
    // モールスから短符数でdecASCII
    var morsejoin=morse.replace(/[\s]/g, "");
    if (morsejoin.match(/\./) && morsejoin.match(/-/) && morsejoin.match(/\.{4}/)) {
  ...
  // モールスから長符数でdecASCII
    ...
    htmlTmp.push("spreadsheetに貼り付けて要確認"); 
    var aa=morse.replace(/\s/g, "");
    aa=aa.replace(/\./g, "＿");
    aa=aa.replace(/-/g, "＠");
    if (aa.length%5==0||aa.length%7==0) {
        if (aa.length%5==0) {
            var wRE=new RegExp("\.{"+aa.length/5+"}", "g");
        } else if (aa.length%7==0) {
            var wRE=new RegExp("\.{"+aa.length/7+"}", "g");
        }
        htmlTmp.push(aa.match(wRE).join("\n"));
        htmlTmp.push("(反転)"); 
        aa=aa.replace(/＿/g, "#");
        aa=aa.replace(/＠/g, "＿");
        aa=aa.replace(/#/g, "＠");
