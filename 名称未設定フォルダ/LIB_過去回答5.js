...
if (
  ...
  for (var i=2; i<=9; i++) { //skip2～9
    ...
    //1文字目
    result.push(TEXT[pos-1]); //indexは0
...
if (
  ...
  htmlTmp.push(TEXT);
  htmlTmp.push("<b>(同じ文字列長の3行をrect)</b>");
  
  function therrLineRectRead(str) {
    if (str.length%3==0) {
      var rect=[];
...
if (
  ...
  function therrLineAutokey(str, str2) {
    if (str.length%3==0) {
  ...
  var tmpjoin=tmp[0]+tmp[1]+tmp[2];
  htmlTmp.push("([1] [2] [3]の順に連結)");
  therrLineAutokey(tmpjoin, tmp[2]);
  
  var tmpjoin=tmp[0]+tmp[2]+tmp[1];
  htmlTmp.push("([1] [3] [2]の順に連結)");
  therrLineAutokey(tmpjoin, tmp[1]);
  ...
  var tmpjoin=tmp[1]+tmp[2]+tmp[0];
  htmlTmp.push("([2] [3] [1]の順に連結)");
  therrLineAutokey(tmpjoin, tmp[0]);
