...
function analyzeText(str) {
  ...
  for (var i in charlist) {
    ...
    resulttmp+="</td><td>";
    resulttmp+=charlist[i].charCodeAt(
      ).toString(10);
    resulttmp+="</td><td>";
    resulttmp+=charlist[i].charCodeAt(
      ).toString(16);
  ...
  result.push("フォーマット通りならキーワード置換やモールスを試す");
  ...
  result.push("<a href='https://niantic.schlarp.com/investigation:apps:ingress:events'>[EVENTS]</a>2013 Freemisty, 2013 IngressDays, 2013 TimeZero, 2013 SaveKlue, 2013 Voynich, 2013 Minotaur, 2013 Cassandra, 2013 13magnus, 2014 Recursion, 2014 Interitus, 2014 IngressIO, 2014 Helios, 2014 Darsana, 2015 Shōnin, 2015 Persepolis, 2015 Abaddon, 2016 Obsidian, 2016 Aegis Nova, 2016 Via Lux, 2016 Via Noir, 2017 exogenous");
  result.push("(よくあるLeet) 1iL, 3E, 4A, 5vS, 6b, 7TL, 9g, 10x");
  ...
  if (str.match(/([a-z]\w{10}11|11[a-z]\w{10})/)) {
    alertMsg.push(
      "11のヒントはyoutubeのIDの可能性あり");
      var ytlink=str.match(
        /([a-z]\w{10}11|11[a-z]\w{10})/)[0].replace(/11/,"");
    alertMsg.push("<a href='vnd.youtube://"+ytlink+"'>http://youtu.be/"+ytlink+"</a>");
  }
  if (str.match(/\d+[.\/\\\-|,%]\d+/g)) {
  ...
  if (tmp) {
    ...
    alertMsg.push("<a href='https://www.fourmilab.ch/earthview/lunarform/cratall.html'>Lunar Craters</a>");
  }
  if (str.match(
...
function leetKeyws() {
  ...
  */
  ...
  ["GIST","https://gist.github.com/anonymous/",""], 
...
function htmlHeader() {
  ...
  h+=".AA {line-height: 83%; letter-spacing: -2.5px;}\n";
  h+=".AA2 {line-height: 100%; letter-spacing: 0px;}\n";
